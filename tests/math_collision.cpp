...
bool rect_point(Number rx, Number ry, Number rwidth, Number rheight, Number px,
                Number py) {
    return check_collision_rect_point<Number>(rx, ry, rwidth, rheight, px, py);
}

template <SignedNumberT Number>
bool rect_circle(Number rx, Number ry, Number rwidth, Number rheight, Number cx,
                 Number cy, Number cr) {
    return check_collision_rect_circle<Number>(rx, ry, rwidth, rheight, cx, cy,
                                               cr);
}
...
TEST_CASE("Collision; rect vs point; U32 - integer cases") {
    ...
    SUBCASE("point strictly inside") {
        CHECK(rect_point<I32>(rx, ry, rw, rh, 5, 2));
    }
    ...
    SUBCASE("point on edges and corners") {
        ...
        CHECK(rect_point<I32>(rx, ry, rw, rh, rx + rw,
                              ry + rh));  // bottom-right corner
...
TEST_CASE("Collision; rect vs point; F32 - floating point cases") {
    ...
    SUBCASE("inside and boundary") {
        ...
        CHECK(rect_point<F32>(rx, ry, rw, rh, rx + rw,
                              ry + rh));  // on bottom-right corner
    ...
    SUBCASE("outside checks") {
        ...
        CHECK(!rect_point<F32>(rx, ry, rw, rh, rx + rw + 0.1f,
                               0.0f));  // right outside
        CHECK(
            !rect_point<F32>(rx, ry, rw, rh, 0.0f, ry - 0.1f));  // above / top
        CHECK(!rect_point<F32>(rx, ry, rw, rh, 0.0f,
                               ry + rh + 0.01f));  // below / bottom
    }
}
...
TEST_CASE("Collision; rect vs circle; F64 - basic cases") {
    ...
    SUBCASE("circle fully inside or overlapping") {
        ...
        CHECK(rect_circle<F64>(rx, ry, rw, rh, 9.5, 5.0,
                               1.0));  // overlapping right edge
        CHECK(rect_circle<F64>(rx, ry, rw, rh, 10.5, 10.5,
                               1.0));  // overlapping corner
    }

    SUBCASE("circle outside / touching") {
        CHECK(!rect_circle<F64>(rx, ry, rw, rh, 20.0, 20.0, 1.0));  // far away
        CHECK(rect_circle<F64>(rx, ry, rw, rh, 11.0, 5.0,
                               1.0));  // touching right edge
        CHECK(!rect_circle<F64>(rx, ry, rw, rh, 12.1, 5.0,
                                1.0));  // outside right edge
    }
}
