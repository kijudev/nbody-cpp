...
template void integrate_body_euler_semi_symplectic(BodyT<F32>& body, F32 dt);
template void integrate_body_euler_semi_symplectic(BodyT<F64>& body, F64 dt);

template <FloatT Float>
void integrate_body_verlet(BodyT<Float>& body, Float dt) {
    // NOTE: Velocity Verlet (simplified version)
    // Full Velocity Verlet would need acceleration at t+dt, but our
    // architecture computes all accelerations before integration. This is a
    // reasonable approximation.
    const Float half  = 0.5;
    const Float dt_sq = dt * dt;

    body.pos =
        body.pos.add(body.vel.scale(dt)).add(body.acc.scale(half * dt_sq));
    body.vel = body.vel.add(body.acc.scale(dt));
}
...
void integrate_body_verlet_proper(BodyT<Float>& body, Float dt,
    ...
    body.pos =
        body.pos.add(body.vel.scale(dt)).add(old_acc.scale(half * dt_sq));
    body.vel = body.vel.add(old_acc.add(body.acc).scale(half * dt));
}

template void integrate_body_verlet_proper(BodyF32& body, F32 dt,
                                           const math::Vec2F32& old_acc);
template void integrate_body_verlet_proper(BodyF64& body, F64 dt,
                                           const math::Vec2F64& old_acc);
