...
template std::vector<math::Vec2F32> generate_position_distribution_uniform_box(USize n, F32 radius);
template std::vector<math::Vec2F64> generate_position_distribution_uniform_box(USize n, F64 radius);
...
template std::vector<math::Vec2F32> generate_position_distribution_uniform_disk(USize n,
                                                                                F32   radius);
template std::vector<math::Vec2F64> generate_position_distribution_uniform_disk(USize n,
                                                                                F64   radius);

// NOTE: Generate position distribution using Plummer model. The center of the Globular Cluster
// is at the origin (0, 0). X = rand(0, 1) r = Rmax * (1 / sqrt(X ** (-2/3) - 1)) theta =
// rand(0, 2pi) NOTE: Convert from polar to Cartesian coordinates. x = r * cos(theta) y = r *
// sin(theta)
template <FloatT Float>
...
template std::vector<math::Vec2F32> generate_position_distribution_plummer_model(USize n,
                                                                                 F32   radius);
template std::vector<math::Vec2F64> generate_position_distribution_plummer_model(USize n,
                                                                                 F64   radius);
...
template std::vector<F32> generate_mass_distribution_uniform(USize n, F32 min_mass, F32 max_mass);
template std::vector<F64> generate_mass_distribution_uniform(USize n, F64 min_mass, F64 max_mass);
...
template std::vector<F32> generate_mass_distribution_salpeter_imf(USize n, F32 min_mass,
                                                                  F32 max_mass);
template std::vector<F64> generate_mass_distribution_salpeter_imf(USize n, F64 min_mass,
                                                                  F64 max_mass);
...
template std::vector<BodyF32> generate_distribution(const GenerateDistributionConfig<F32>& config);
template std::vector<BodyF64> generate_distribution(const GenerateDistributionConfig<F64>& config);
