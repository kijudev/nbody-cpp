...
template <FloatT Float>
Camera<Float>::Vec2 Camera<Float>::world_to_screen_vec(
    const Vec2& world_pos) const {
...
Camera<Float>::viewport_extent_world() const {
    return std::make_tuple(screen_to_world({0, 0}),
...
template <FloatT Float>
bool Camera<Float>::is_circle_in_viewport(const Vec2& center,
                                          Float       radius) const {
    Vec2 w0      = screen_to_world({0, 0});
    Vec2 wscreen = screen_to_world({screen_width, screen_height});
...
template <FloatT Float>
void Camera<Float>::handle_controls_movement(Float dt) {
    // NOTE: RIGHT / LEFT
...
template <FloatT Float>
void Camera<Float>::handle_controls_zoom(Float dt) {
    zoom = static_cast<Float>(
        std::expf(std::logf(static_cast<F32>(zoom)) +
